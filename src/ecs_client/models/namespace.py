...
class NamespaceData:
    ...
    def from_xml(xml_data: bytes) -> "NamespaceData":
        ...
        return NamespaceData(
            ...
            default_bucket_block_size=int(
                root.find("default_bucket_block_size").text
            ),
            block_size_in_count=int(root.find("blockSizeInCount").text),
            block_size=int(root.find("blockSize").text),
            is_compliance_enabled=get_bool(
                root.find("is_compliance_enabled").text
            ),
            is_encryption_enabled=get_bool(
                root.find("is_encryption_enabled").text
            ),
            ...
            is_object_lock_with_ado_allowed=get_bool(
                root.find("is_object_lock_with_ado_allowed").text
            ),
            notification_size_in_count=int(
                root.find("notificationSizeInCount").text
            ),
            notification_size=int(root.find("notificationSize").text),
            default_data_services_vpool=root.find(
